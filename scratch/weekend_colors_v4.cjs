...
// 1. Replace TH block
const thStartMarker = '<tr style={{background:"var(--bg-panel)"}}>\n              {weeks.map((w,wi) => (\n                w.days.map((d,di) => (';
// Let's just find the exact index
const thIdx = pilotHrsBody.indexOf('              {weeks.map((w,wi) => (\r\n                w.days.map((d,di) => (\r\n                  <th key={d}');
if (thIdx !== -1) {
    const thEndIdx = pilotHrsBody.indexOf('              ))}', thIdx) + 17;
    const oldTh = pilotHrsBody.substring(thIdx, thEndIdx);
    const newTh = `              {weeks.map((w,wi) => (
                w.days.map((d,di) => {
    ...
    pilotHrsBody = pilotHrsBody.replace(oldTh, newTh);
} else {
    throw new Error("Could not find TH block");
}

// 2. Replace TD block
const tdIdx = pilotHrsBody.indexOf('                  {weeks.map((w,wi) => (\r\n                    w.days.map((d,di) => {\r\n                      const dh = dailyHrs[d];');
if (tdIdx !== -1) {
    const tdEndIdx = pilotHrsBody.indexOf('                  ))}', tdIdx) + 21;
    const oldTd = pilotHrsBody.substring(tdIdx, tdEndIdx);
    const newTd = `                  {weeks.map((w,wi) => (
                    w.days.map((d,di) => {
                      const dh = dailyHrs[d];
                      const dow = new Date(year, month, d).getDay();
                      const isSat = dow === 6;
    ...
    pilotHrsBody = pilotHrsBody.replace(oldTd, newTd);
} else {
    throw new Error("Could not find TD block");
}

// 3. Replace ExcelJS block
const exIdx = pilotHrsBody.indexOf('    ws.columns.forEach((col, idx) => {\r\n        if (idx === 0) col.width = 8;');
if (exIdx !== -1) {
    const exEndIdx = pilotHrsBody.indexOf('    });\r\n\r\n    const buf = await wb.xlsx.writeBuffer();', exIdx);
    const oldEx = pilotHrsBody.substring(exIdx, exEndIdx + 7);
    const newEx = `    ws.columns.forEach((col, idx) => {
        if (idx === 0) col.width = 8;
    ...
    pilotHrsBody = pilotHrsBody.replace(oldEx, newEx);
} else {
    throw new Error("Could not find ExcelJS block");
}

fs.writeFileSync('src/squadron-dashboard.tsx', before + pilotHrsBody);
console.log('Successfully replaced 3 blocks using substring logic');
