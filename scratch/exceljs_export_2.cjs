...
const targetStart = c.indexOf('onClick={()=>{try{const table = document.getElementById("pilot-hrs-table-" + acType);');
const targetEnd = c.indexOf('📥 Export Excel</button>', targetStart);

if (targetStart !== -1 && targetEnd !== -1) {
    const oldCode = c.substring(targetStart, targetEnd);
...
}} style={{padding:"6px 12px",borderRadius:6,border:"none",background:"#10b981",color:"#fff",cursor:"pointer",fontWeight:700,fontSize:13}}>`;

    // Wait, the string exists TWICE! Once in PilotHrsTab, once in PostFlightTab.
    // So we must replace ALL occurrences!
    const parts = c.split(oldCode);
    const updated = parts.join(newCode);
