...
const idx = c.indexOf('function PilotHrsTab() {');
if (idx === -1) {
...
after = after.replace(/setLogs\(pfRows\.slice\(1\)\.map\(r=>\(\{\r?\n.*?\r?\n.*?\r?\n.*?\r?\n\s+\}\)\)\);\r?\n\s+\}/, (match) => {
  return match + `
...
after = after.replace(/<div style=\{\{padding:20\}\}>\r?\n\s+<\/div>/, `<div style={{padding:20}}>
          {view === "hours-92a" 
...
fs.writeFileSync('src/squadron-dashboard.tsx', before + after);
console.log('Regex replace done safely');
